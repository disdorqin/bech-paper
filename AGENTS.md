...
Current mode: **UNIFIED DA S1 SHAPE PASSED / HSA REJECTED / HOST-RELATIVE STATE INTERACTION REJECTED / S1 + POOLED FIXED ALPHA FROZEN / PAPER PREWRITING AUTHORIZED / NO ACTIVE SCIENTIFIC EXECUTION**.
...
The Unified-DA Shape Engineering canary is complete and adjudicated. S1 Daily-Patch GRU32 Shape is the leading Shape candidate: median relative Overall-MAE gain 5.85%, 3/6 cells >=5%, maximum 10.74%, but PJM/PatchTST is a real negative cell. Shape-only conditional Amplitude, state-intensity -> distance, state-excursion Shape, and direct horizon-aligned state residual are all closed. HSA genuinely helps both GANSU_DA Hosts (Shape cosine `+0.0666`/`+0.1170`, Overall-MAE `+1.30`/`+4.31` pp vs S1) but is not universal. Its learned coefficients reveal a transferability conflict: the same semantic role can require opposite residual correction across markets because HCH repairs the **Host residual**, not price directly.

The terminal structural closure `docs/current/HCH_HOST_RELATIVE_STATE_INTERACTION_FINAL_CLOSURE_20260911.md` has been executed and returned `HCH_HOST_RELATIVE_STATE_INTERACTION_NOT_SUPPORTED_FREEZE_S1` (R0/R4 `PASS`, R1/R2/R3 `FAIL`). It froze S1, built a normalized recent Host-bias direction from the already-available residual/mask history, multiplied it by the fixed five-role standardized state table, and learned exactly **one global five-parameter beta shared across all six development cells** with equal total Shape-loss weight per cell (`u_HRSI=normalize(u_S1+Z beta)`). The shared beta destroys the GANSU_DA mechanism HSA had: GANSU Shape cosine changes by only `+0.0002` / `-0.0105` against HSA's `+0.0666` / `+0.1170`, Overall-MAE gain vs S1 is nonnegative in only 1/6 cell medians, and 0/4 international cells are strictly better. Evidence: `experiments/evidence/hch_host_relative_state_interaction_20260911/`, independently reproduced by `verify_gates.py` (50/50 checks).

This stage was terminal and is now closed. **`S1 Daily-Patch GRU32 Shape + pooled fixed alpha` is frozen as the final structural candidate.** No further structural rescue is authorized: per-market/per-Host beta, per-hour beta, role selection, conditional Amplitude, alternate windows or transforms, and post hoc threshold relaxation are all excluded. The next stage is human adjudication, then the full public-China/international comparison against the completed baseline-fidelity suite. Do not start that comparison or any successor structure without fresh human authorization.
