"""One-shot 16-cell PROTECTED_FINAL confirmation for the frozen S1-B2 method.
...
parsed; ``S.unlock_protected()`` refuses to open the seal without that token.

What it does, in the freeze's own terms:
...
* **Fit stays ``POST_TRAIN``; scoring becomes ``PROTECTED_FINAL``.**  The fit role
  is the frozen one; only the scoring partition moves.  A protected day's 168h
  residual history legitimately reaches back into ``DEV_EVAL`` and into earlier
  protected days -- those targets were observed before the day being forecast --
  and nothing is fitted, tuned, rescaled or checkpointed on them.
...
* **Frozen thresholds.**  q05 / q95 / S90 are the ones the frozen family stages
  froze in ``00_protocol/THRESHOLD_FREEZE.json`` before any cell existed; they are
  re-used verbatim, never recomputed here.
* **Cell statistic.**  Median MAE across the freeze's HCH seeds 7/17/37; every gain
  is computed after that median, never before it.
...
1. The frozen Hosts persisted **no weights** and evaluated the open roles only, so
   ...
   forwarding the re-derived frozen weights that P0 proved digest-identical --
   never by fitting a Host here.
2. ``panel.rebuild_series`` enumerates its evaluation span by the legacy role name
   ...
   frozen ``pred.shape == (n_eval, H)`` invariant still holds and the evaluation
   span is exactly the protected days.  PIR's train/val spans and its scaler are
   provably unchanged by this: both depend only on ``n_fit``, ``n_val`` and the
   fit block.

Nothing here re-ranks, rescues or selects.  The complete 16-cell matrix is
executed in one pass; there is no conditional stopping.
"""
...
# The freeze's flag vector, verbatim (section 3).  Not a tunable.
...
METRIC_KEYS = ("Overall_MAE", "MSE", "RMSE", "Host_Overall_MAE",
               ...
               "Upper_tail_day_MAE", "upper_tail_day_MAE_status",
               ...
               "n_normal_hours", "n_extreme_days", "n_negative_hours", "n_negative_days")
...
def main(argv: list[str]) -> int:
    ...
    log("=" * 78)
    log("S1-B2 international confirmation -- PROTECTED_FINAL, one pass, 16 cells")
    ...
    # ==== pass A: S1-B2 + the two non-PIR comparators + COSA, all 16 cells =====
    for market in S.MARKETS:
        ...
        for host in S.HOSTS:
            ...
            cand = {"Host": host_mae,
                    "delta-Adapter": float(np.mean(np.abs(y - bpreds["delta-Adapter"])))}
            cell_rows.append({
                ...
                "Host_MAE": host_mae,
                "delta_Adapter_MAE": cand["delta-Adapter"],
    ...
    S.dump(OUT / "S1B2_PANEL.json", {"panel": panel, "cells": cell_rows,
                                     "token": verdict,
                                     "flags": FLAGS,
                                     "seeds": list(S.SEEDS),
                                     "transfer_seed": TRANSFER_SEED,
                                     "pir_seed": PIR_SEED, "cosa_seed": COSA_SEED})
    S.dump(OUT / "PROTECTED_ACCESS_AUDIT.json", {
        "schema": "hch_s1_b2_international_protected_access.v1",
    ...
    S.dump(OUT / "VERDICT.json", {
        "schema": "hch_s1_b2_international_verdict.v1",
        "token": verdict["token"],
...
def _extended_relabel(market: str) -> dict:
    ...
    Only PIR consumes the reconstructed sliding-window series, and it enumerates
    its evaluation span by legacy role name.  ``DEV_EVAL`` is sent to the same
    unused legacy name the frozen map gives ``PROTECTED_FINAL`` so that the
    evaluation span is exactly the protected days -- the frozen
    ``pred.shape == (n_eval, H)`` invariant then holds unchanged.  PIR's train and
    validation spans are untouched: both are derived from ``n_fit``/``n_val`` and
    the fit block alone.
    """
...
def _median_row(seeds: list[dict]) -> dict:
    """The cell statistic: median across the freeze's seeds, taken *before* any gain."""
...
def _build_protected_cell(market: str, host: str, HIST) -> tuple[dict, object, dict, dict]:
    ...
    where ``pack`` carries the sealed arrays and the frozen ``build_cell`` output
    for the non-PIR comparators.  Every one of them is scored on exactly these days.
    """
    open_cache, cache_path = S.load_host_cache(market, host)
    w_sealed, pred_sealed, digest = S.predict_role(market, host, S.EVAL_ROLE)
    ...
    pack = {"full": full, "timestamp": w_sealed.timestamp,
            ...
            "hp": pred_sealed.astype(np.float64)}
...
def cosa_cut_plan(n_days: int, max_cuts: int = COSA_MAX_CUTS) -> list[int]:
    ...
    every one of up to 653 cuts here would cost hours of purely confirmatory work,
    so the cuts are thinned by a uniform stride fixed by this rule alone -- computed
    from the day count before any prediction exists, never from a result.  The last
    cut is always included: it is the deepest prefix test available.
    """
...
def _cosa_chronology(cosa_transfer, hp, y, seed_means) -> dict:
    ...
    Poisoning every day from k onward must leave predictions 0..k untouched
    (prefix cleanliness) and must change at least one later day (liveness), or the
    test is vacuous.
    """
...
def _aggregate(cells: list[dict], base_rows: list[dict],
    ...
    host_win = sum(1 for c in cells if c["beats_Host"])
    bs_win = sum(1 for c in cells if c["beats_best_static"])
    ...
    return {
        ...
        "all_16_cells_scored": bool(n == len(S.CELLS) and not failures),
        "host_positive_count": int(host_win),
        "best_static_win_count": int(bs_win),
        "panel_median_host_gain_pct": med(gains),
        "panel_median_gain_vs_best_static_pct": med(bs_gains),
        "worst_cell_host_gain_pct": float(min(gains)),
        "best_cell_host_gain_pct": float(max(gains)),
...
def _token(panel: dict) -> dict:
    """The freeze's section 8 claim-strength rule, applied verbatim.

    ``hard_fail`` is a source / access / chronology failure, not a weak result: a
    cell that could not be scored, a PIR row that is not legally available on its
    Host, or a COSA chronology audit that fails all make the stage INVALID rather
    than merely unimpressive.
    """
    hard_fail = (not panel["all_16_cells_scored"]
                 or panel["n_cells"] != len(S.CELLS)
                 or panel["protected_days_total"] <= 0
                 or panel["cosa_chronology_failures"] > 0)
    strong = (panel["host_positive_count"] >= 12
              and panel["panel_median_host_gain_pct"] >= 1.0
              and panel["best_static_win_count"] >= 10
              and panel["market_medians_host_positive"] >= 3
              and panel["worst_market_median_host_gain_pct"] >= -3.0)
    positive = (panel["host_positive_count"] >= 9
                and panel["panel_median_host_gain_pct"] > 0.0
                and panel["market_medians_host_positive"] >= 2)
    gates = {
        "host_positive_count>=12": panel["host_positive_count"] >= 12,
        "panel_median_host_gain>=+1.0%": panel["panel_median_host_gain_pct"] >= 1.0,
        "best_static_wins>=10": panel["best_static_win_count"] >= 10,
        "market_medians_host_positive>=3": panel["market_medians_host_positive"] >= 3,
        "worst_market_median>=-3.0%": panel["worst_market_median_host_gain_pct"] >= -3.0,
        "positive:host_positive_count>=9": panel["host_positive_count"] >= 9,
        "positive:panel_median_host_gain>0": panel["panel_median_host_gain_pct"] > 0.0,
        "positive:market_medians_host_positive>=2": panel["market_medians_host_positive"] >= 2,
        "no_source_access_chronology_failure": not hard_fail,
    }
    if hard_fail:
        tok = "HCH_S1_B2_INTERNATIONAL_INVALID"
    elif strong:
        tok = "HCH_S1_B2_INTERNATIONAL_STRONG_UNTOUCHED_GENERALIZATION"
    elif positive:
        tok = "HCH_S1_B2_INTERNATIONAL_POSITIVE_UNTOUCHED_GENERALIZATION"
    else:
        tok = "HCH_S1_B2_INTERNATIONAL_MIXED_UNTOUCHED_GENERALIZATION"
    return {"token": tok, "gates": gates,
            "rule": "freeze section 8; changes claim wording only, never authorises redesign"}
