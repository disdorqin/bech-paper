"""Exp2b: EPEX-DE + PJM × 2 backbones × all 5 baselines vs HCH."""
...
class PIR_Simple:
    ...
    def fit(self, Z, yhat, y):
        ...
        resid = y - yhat; self.model = Ridge(alpha=1.0).fit(Z, resid.mean(axis=1) if resid.ndim > 1 else resid)
...
BASELINES = [Identity(), QuantileCorrection(seed=0), VahediStyle(seed=0), CRC(seed=0), PIR_Simple(seed=0), SpikeRegularization(seed=0)]
DATASETS = ["DE_EPEX", "PJM_2020"]
BB_LIST = ["Linear", "GBDT"]
...
for ds_key in DATASETS:
    ...
    for bb_name in BB_LIST:
        ...
        yhat_full = bb.predict(X); y_base = yhat_full[s4]
        ...
        y_hch, diag = hch.apply(s4_z, y_base)

        bb_res = {}
        bb_res["Base"] = all_metrics(y_true, y_base, y_base)
        bb_res["HCH"] = all_metrics(y_true, y_hch, y_base)
        bb_res["HCH"]["fire"] = diag["fire_rate"]
        bb_res["HCH"]["lam_neg"] = diag["lam_neg"]
...
out_path = OUT / "exp2b_epex_pjm.json"
