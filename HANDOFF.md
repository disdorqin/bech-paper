...
## Current handoff — O1 full-8 panel completion returned `COMPLETE_FOR_ADJUDICATION` with `AUXILIARY_PERSISTENCE_BROADLY_HARMFUL_ON_RECOVERY_PANEL`; the panel is closed and nothing follows from it automatically (2026-09-18)

Stage `experiments/current/hch_v44_o1_full8_completion_20260918/` returned **`HCH_V44_O1_FULL8_COMPLETION_COMPLETE_FOR_ADJUDICATION`**. The frozen O1 schedule was extended to the full 8-cell R1 recovery panel with exactly **12 new fits** (`GANSU_DA/LSTM`, `SHANDONG_DA/PatchTST`, `SHAANXI_DA/PatchTST`, `QINGHAI_DA/TimeMixer` × seeds 7/17/37) alongside the 12 reused O1 runs and 24 reused R1 references; nothing already trained was rerun, and the new fits inherit the trainer by import so all 24 runs share one `probe_code_hash`. Independent verifier 16/16 without importing the runner, TEST reads 0, `src/core` unchanged, objective switch exactly after step 400.

**Gate 5/5 → `AUXILIARY_PERSISTENCE_BROADLY_HARMFUL_ON_RECOVERY_PANEL`.** 7/8 cell medians improve, panel median **+0.6793%** (threshold +0.5%), worst cell exactly neutral, no step-0 retreat, selected correction ratio median 0.2244, 18/24 seeds improve. Best cell SHANDONG_DA/PatchTST +2.1831%; the four previously unprobed cells give a **+1.2007%** sub-panel median against +0.1905% for the reused four, so the effect is stronger where it had not been tested. Mechanism: 20/24 runs select post-switch; after step 400 `L_rec` falls in 24/24 runs while `L_B` rises in 24/24; `aux_worse_val_better` occurs in 12/24, and `aux_better_val_worse` never occurs at all.

**What this means, and what it does not.** On the recovery panel, persisting the coordinate auxiliaries for the whole 2000-step budget is broadly worse for VAL MAE than dropping to `L_rec` after step 400 — the GANSU_DA/PatchTST mechanism now has cross-cell support. But this is a TRAIN+VAL-only diagnostic on a recovery panel: not a paper gate, not a TEST result, and not evidence that any component should be deleted. **Do not** remove B/Shape/Level losses, delete the router, simplify architecture, sweep loss weights or the switch step, use PCGrad/GradNorm/SAM, add features, rerun Hosts/baselines, access V2 TEST, or edit `paper/**` on the strength of it. The next action is **main-window adjudication**; no next method experiment is authorized by this stage.

## Current handoff — O1 objective-alignment probe is complete; next and only authorized method experiment is the remaining-four-cell O1 panel completion (2026-09-17)

Stage `experiments/current/hch_v44_objective_alignment_probe_20260917/` returned **`HCH_V44_OBJECTIVE_ALIGNMENT_PROBE_COMPLETE_FOR_ADJUDICATION`** after 12/12 TRAIN+VAL fits, independent verification 13/13 and TEST read count 0. O1 keeps v4.4 G2 unchanged but uses the frozen full coordinate loss only through optimizer step 400 and `L_rec` alone from step 401 onward. Against reused R1 full-loss references, cell-median VAL gains are GANSU_DA/PatchTST **+1.7337%**, NINGXIA_DA/iTransformer +0.1907%, SHANDONG_DA/iTransformer +0.1903%, SHAANXI_DA/TimeMixer +0.0000%; panel median **+0.1905%**. The preregistered +0.5% broad-harm gate fails, so the formal verdict is `AUXILIARY_PERSISTENCE_NOT_SHOWN_HARMFUL`, not a global rejection of the coordinate auxiliaries.

The mechanism evidence nevertheless changes the diagnosis: GANSU_DA/PatchTST full-loss R1 improves TRAIN reconstruction, balanced-mass error and Shape W1 while VAL degrades monotonically; after O1 turns off coordinate auxiliaries, VAL improves materially even while balanced-mass auxiliary error becomes much worse. Thus optimization starvation is not the whole problem, and exact residual geometry does not make persistent component-wise supervision automatically MAE-aligned.

**Next authorization is intentionally narrow.** Run the exact same O1 schedule on only the four missing cells of the frozen R1 recovery panel: `GANSU_DA/LSTM`, `QINGHAI_DA/TimeMixer`, `SHAANXI_DA/PatchTST`, `SHANDONG_DA/PatchTST`, seeds 7/17/37 = **12 new fits**. Reuse the existing four-cell O1 runs and all R1 references; do not retrain them. Apply the same TRAIN+VAL-only TEST quarantine. The 8-cell diagnostic gate is the prior O1 gate generalized without tuning: ≥6/8 cell medians improve, panel median O1-vs-R1 gain ≥0.5%, worst degradation ≤0.5%, non-degenerate correction/no result-driven step-0 retreat, and no leakage/numerical/protocol violation.

Until that panel is complete, do **not** remove B vs Shape auxiliaries separately, delete the router, change coordinate capacity, sweep loss weights/switch steps, use PCGrad/GradNorm/SAM, add features, rerun Hosts/baselines, access V2 TEST, or edit `paper/**`. If the 8-cell O1 schedule does not generalize, return to main-window adjudication; if it does, only then decide whether the next single-factor study is component-loss attribution or architectural simplification.
