"""The single shared feature-interaction stem.
...
class UnifiedFeatureStem(nn.Module):
    ...
    The module maps ``(..., D)`` to ``(..., out)``, so passing ``(B, H, D)``
    yields one embedding per hour.  The reference default is
    ``Linear(D,64) -> GELU -> LayerNorm -> Linear(64,32)``.  Widths are
    engineering choices, not scientific claims.
    """
