"""Deterministic balanced fusion, and the exact targets it is trained against.

The two branches meet through the factorization, never through a learned fusion
layer.  The residual decomposition that defines the training targets is the
general signed-mass identity::
...
The active *candidate* is the balanced special case that survived adjudication::

    c_h = A (S+_h - S-_h),      A >= 0

which satisfies ``sum_h c_h = 0`` exactly, up to the numerical accuracy of the
two softmax normalizations.  The candidate is therefore a within-day residual
redistribution, not a level correction.

Zero mass is handled explicitly: when ``A = 0`` the corresponding Shape is set to
the uniform distribution and the mass to zero, so the product is exactly zero and
no ``0/0`` can appear.
"""
...
from .contracts import ShapeOutput, SignedMassTargets

__all__ = ["safe_shape", "decompose_residual", "fuse", "repair", "correction_mass"]
...
def decompose_residual(residual: torch.Tensor,
    ...
    ``residual`` is ``(..., H)``.  Masked steps are zeroed before the masses are
    formed, so they contribute to neither sign.
    """
...
def fuse(amplitude: torch.Tensor, shape: ShapeOutput) -> torch.Tensor:
    """``c_h = A * (S+_h - S-_h)``.  No learned parameters, no second head."""
    return amplitude.unsqueeze(-1) * (shape.positive - shape.negative)


def correction_mass(correction: torch.Tensor) -> torch.Tensor:
    """``sum_h c_h`` over the horizon.  Must be zero for a balanced candidate."""
