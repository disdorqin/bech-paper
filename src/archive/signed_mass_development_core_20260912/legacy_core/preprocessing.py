"""Deterministic canonicalisation for the signed-mass repair core.
...
Two task views are then created from the same facts:

* an absolute, training-standardised base token for the ONE shared MLP;
* additional scale-free within-day profiles and ramps for Shape.

Historical residuals are deliberately split into two representations:
Shape history removes scale; Amplitude history retains scale.  This fixes the
old failure mode where a normalised Shape tensor was accidentally reused as
historical magnitude information.
"""
...
class TrainFrozenScaler:
    ...
    ``mask`` may be supplied to prevent absent semantic roles from affecting the
    fitted statistics.  Missing entries are returned as exact zeros after
...
def base_token_dim(n_forecast_features: int, n_calendar_features: int = 0) -> int:
    """Width of ``[host, features, calendar, host_mask, feature_masks]``."""
...
def shape_channel_dim(n_forecast_features: int, n_calendar_features: int = 0) -> int:
    """Width of privileged Shape geometry channels.

    host profile+ramp; each feature profile+ramp; horizon position; all calendar
    encodings; host-valid mask; one availability mask per forecast feature.
    """
    if n_forecast_features < 0 or n_calendar_features < 0:
        raise ValueError("feature counts must be non-negative")
    return 4 + 3 * n_forecast_features + n_calendar_features
...
class DeterministicFeatureBuilder:
    """Create the two task views without market-specific feature logic.

    All target-day legal numeric covariates enter the same shared base token.
    Shape additionally receives normalised profile/ramp coordinates for every
    available covariate.  Amplitude receives the absolute shared embedding and a
    *separate* scale-sensitive residual-history tensor in the model.
    """
    ...
    def build(self, raw: RawInputs) -> ForecastOriginBatch:
        ...
        # Shape receives privileged scale-free geometry for every available
        # target-day profile, not a hand-picked market-specific feature list.
        host_profile = within_day_robust_normalize(host, valid)
        host_ramp = first_difference(host_profile, valid)

        ft = features.transpose(1, 2)              # (B,F,H)
        fm = feature_mask.transpose(1, 2) * valid.unsqueeze(1)
        feature_profile = within_day_robust_normalize(ft, fm)
        feature_ramp = first_difference(feature_profile, fm)

        position = torch.linspace(-1.0, 1.0, horizon, dtype=host.dtype,
                                  device=host.device).expand(batch, horizon)
        calendar_ch = calendar.transpose(1, 2)
        shape_channels = torch.cat([
            host_profile.unsqueeze(1),
            host_ramp.unsqueeze(1),
            feature_profile,
            feature_ramp,
            position.unsqueeze(1),
            calendar_ch,
            valid.unsqueeze(1),
            feature_mask.transpose(1, 2),
        ], dim=1)

        expected_shape = shape_channel_dim(n_features, calendar.shape[-1])
        if shape_channels.shape[1] != expected_shape:
            raise RuntimeError("Shape channel dimension contract violated")
